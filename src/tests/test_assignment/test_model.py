...
def test_due_datetime_can_be_iso_string(schedule_in_db):
    ...
    assignment = Assignment(task=schedule_in_db.task, scheduler=schedule_in_db, due_datetime=expected_date_str)
...
def test_due_datetime_iso_string_must_be_valid(schedule_in_db):
    with pytest.raises(ValueError):
        Assignment(task=schedule_in_db.task, scheduler=schedule_in_db, due_datetime="aaaaaaa")

def test_due_datetime_must_be_utc(schedule_in_db):
    with pytest.raises(ValueError):
        Assignment(task=schedule_in_db.task, scheduler=schedule_in_db, due_datetime=datetime.now())

def test_submission_date_can_be_iso_string(schedule_in_db):
    expected_date = datetime.now(UTC) + timedelta(days=1)
    expected_date_str = expected_date.isoformat()

    assignment = Assignment(task=schedule_in_db.task, scheduler=schedule_in_db, due_datetime=datetime.now(UTC),
...
def test_submission_date_iso_string_must_be_valid(schedule_in_db):
    with pytest.raises(ValueError):
        Assignment(task=schedule_in_db.task, scheduler=schedule_in_db, due_datetime=datetime.now(UTC),
